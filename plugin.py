...
class Main:
    ...
    def LoadInfos(self):
        ...
        if self.info == 'getnextepisodes':
            ...
            # Ak sú ID prázdne, alebo ide o neplatný text, pošleme Kodi prázdny zoznam
            if not channel_ids or channel_ids.strip() == '':
                xbmcgui.Window(10700).clearProperty('channel_change')
                xbmcplugin.endOfDirectory(handle=self.widget_handle, succeeded=False)
                return
