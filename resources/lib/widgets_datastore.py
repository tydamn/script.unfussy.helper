...
class WidgetsDataStore:
    ...
    def setSkinStrings(self):
        ...
        for widget in self.widgets:
            ...
            if widget['category'] == 0 and widget['type'] == 3:
                name = str(widget['header'])
                base_path = self.wm.getPath(0, 3)
                path = base_path + '&pointintime=' + str(widget['pointintime'])
                path += '&channels='
    ...
    def getValue(self, index_widget, item):
        if item not in self.widgets[index_widget]:
            return
    ...
    def switchElements(self, index_widget, up):
        ...
        if up:
            ...
            if index_widget == len(self.widgets) - 1:
                return
            index_new = index_widget + 1
...
class WidgetXMLWriter:
    ...
    def setParam(self, parent, name, value):
        ...
        param.set('name', name)
        param.text = '' if value is None else str(value)
    ...
    def writeStaticContent(self, parent, widget, widget_id):
        ...
        for addon in widget['addons']:
            ...
            label.text = '' if addon['name'] is None else str(addon['name'])

            thumb = xml.SubElement(item, 'thumb')
            thumb.text = '' if addon['thumb'] is None else str(addon['thumb'])

            onclick = xml.SubElement(item, 'onclick')
            onclick.text = 'RunAddon(' + str(addon['id']) + ')'

    def createWidgetHeaderCond(self, parent, widgets):
        id = 500

        conditions = ['ControlGroup(9002).HasFocus']

        for widget in widgets:
            if not widget['visible']:
                continue

            conditions.append('Control.HasFocus(' + str(id) + '777)')
            id += 1
